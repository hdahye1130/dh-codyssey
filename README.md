...
| 디버깅 | 각 모듈의 input/output과 bundle을 매우 세밀하게 확인 가능 | step별 실행 결과와 성공/실패 상태를 직관적으로 확인 가능 |
...
- 복잡한 데이터 가공을 Code Step에 집중하면 no-code만으로 내부 로직을 이해하기 어려울 수 있다.
- 데이터 처리 과정을 세부 단계별로 시각화해 추적하는 경험은 Make가 더 강했다.
